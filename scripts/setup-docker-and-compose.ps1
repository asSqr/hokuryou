...
function Enable-WSL2 {
  ...
  # WSL2 default（失敗しても続行）
  & wsl.exe --set-default-version 2 | Out-Null
  if ($LASTEXITCODE -ne 0) {
    Write-Warn "wsl --set-default-version 2 が失敗しました（Windowsが古い/再起動未実施の可能性）。"
  }

  # ここがポイント：例外ではなく ExitCode を見る
  & wsl.exe --status | Out-Null
  $statusExit = $LASTEXITCODE

  # ディストリ有無チェック（--status が通ってもディストリ未導入はよくある）
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0 -and $distros -and ($distros | Where-Object { $_.Trim().Length -gt 0 })) {
      $hasDistro = $true
    }
  } catch { }

  if ($statusExit -ne 0 -or $listExit -ne 0 -or -not $hasDistro) {
    Write-Warn "WSL が未導入 or ディストリ未インストールの可能性があるため wsl --install を試します。"
    # ここは OS によっては --install が無いので ExitCode で判断
    & wsl.exe --install | Out-Null
    if ($LASTEXITCODE -ne 0) {
      Write-Warn "wsl --install が失敗しました。OS が古い/Store版WSLが必要/再起動未実施の可能性があります。"
      Write-Warn "代替: 再起動後に 'wsl --install' を手動実行、または Windows の機能で WSL/VM Platform を有効化してください。"
    }
  } else {
    Write-Info "WSL は導入済み（ディストリも存在）と判断しました。"
  }
}
